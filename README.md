...
- The script ***reads your eBird password from the config file stored in plain text on your computer***.
  ...
  - The password
    - Is read from the cfg file using ```getConfig()```, lines ```5-17```, which is called in line ```153```
    -  ```cfg['pw']``` see ```getConfig()```, lin lines ```131-132```, and is only sent to the eBird login page (see lines ```137-150```. I encourage you to examine the code before running.
